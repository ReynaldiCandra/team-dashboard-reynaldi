...
export function useKpiStats() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [leadsRes, perfRes, sourcesRes] = await Promise.all([
      supabase.from('leads').select('id, status, source, created_at'),
      supabase.from('performances').select('leads_in, revenue, closing, record_date').order('record_date', { ascending: true }),
      supabase.from('leads').select('source'),
    ])

    const leads = (leadsRes.data ?? []) as Record<string, unknown>[]
    const perfs = (perfRes.data ?? []) as Record<string, unknown>[]
    ...
    const totalRevenue = perfs.reduce((s, p) => s + ((p.revenue as number) ?? 0), 0)
    const totalClosing = perfs.reduce((s, p) => s + ((p.closing as number) ?? 0), 0)
    const perfsThisMonth = perfs.filter(p => (p.record_date as string) >= thisMonthStart)
    const perfsLastMonth = perfs.filter(p => { const d = p.record_date as string; return d >= lastMonthStart && d < lastMonthEnd })
    const revenueThisMonth = perfsThisMonth.reduce((s, p) => s + ((p.revenue as number) ?? 0), 0)
    const closingThisMonth = perfsThisMonth.reduce((s, p) => s + ((p.closing as number) ?? 0), 0)
    const revenueLastMonth = perfsLastMonth.reduce((s, p) => s + ((p.revenue as number) ?? 0), 0)
    const closingLastMonth = perfsLastMonth.reduce((s, p) => s + ((p.closing as number) ?? 0), 0)
    const conversionRate = totalLeads > 0 ? Math.round((leads.filter(l => l.status === 'enrolled').length / totalLeads) * 1000) / 10 : 0
    ...
    const allSources = (sourcesRes.data ?? []) as Record<string, unknown>[]
    allSources.forEach(l => { const src = (l.source as string)||'Lainnya'; sourceCounts[src]=(sourceCounts[src]??0)+1 })
    ...
    setStats({ totalLeads, totalRevenue, totalClosing, conversionRate, leadsThisMonth, revenueThisMonth, closingThisMonth, leadsLastMonth, revenueLastMonth, closingLastMonth, monthlyTrend, sourceBreakdown })
    setLoading(false)
  }, []) // eslint-disable-line
  ...
  useEffect(() => {
    ...
    const ch2 = supabase.channel('kpi-perf').on('postgres_changes',{event:'*',schema:'public',table:'performances'},fetchStats).subscribe()
